...
class _LikedPostsScreenState extends State<LikedPostsScreen> {
  // --- STATE DEĞİŞKENLERİ ---
  Future<List<Post>>? _likedPostsFuture;
  ...
  void _loadPosts() {
    ...
    if (auth.isLoggedIn) {
      setState(() {
        _likedPostsFuture = ApiService.getLikedPosts(auth.token!);
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CustomAppBar(title: 'Beğenilen Yazılar'),
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.favorite_border_rounded, size: 64, color: theme.colorScheme.onSurface.withOpacity(0.5)),
            const SizedBox(height: 24),
            Text(
              'Beğendiğiniz yazıları görmek için lütfen giriş yapın.',
  ...
  Widget _buildLoggedInView() {
    // _likedPostsFuture'un null olma ihtimaline karşı (eğer kullanıcı giriş yapmamışsa)
    if (_likedPostsFuture == null) {
  ...
  Widget _buildHeader(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        children: [
          Icon(Icons.favorite_rounded, color: Colors.pink.shade400, size: 40),
          const SizedBox(height: 10),
          Text('Beğenilen Yazılar', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          Text(
            'Daha sonra tekrar göz atmak için favorilerinize eklediğiniz yazılar.',
            textAlign: TextAlign.center,
            style: theme.textTheme.bodyLarge,
          ),
        ],
    ...
    return Column(
      children: [
        ...
        Expanded(
          child: Center(
            child: Text(
              'Henüz hiç yazı beğenmemişsiniz.',
